...
if(!isset($_COOKIE['not_unique1'])) {
    ...
    $sql = "UPDATE likes2 SET views = views + 1 WHERE ID = 1";
...
<video
  ...
  <a href="action-page-ip.php">click</a>
...
function showCustomer(x) {
  ...
  xhttp.open("GET", "views/action-page.php?");
  xhttp.send();
}
</script>
...
<h2><span class="badge rounded-pill text-bg-primary"><?php foreach($data2 as $row): ?>
...
<div class="float-end">
<h1>
<button onclick="showCustomer(this)" class="btn btn-primary"><?php foreach($data as $row): ?>
